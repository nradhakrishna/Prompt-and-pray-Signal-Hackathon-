...
class TaskStore(Protocol):
    ...
    def stage_result(self, result: WorkerResult, now: float) -> Task: ...
...
class InMemoryTaskStore:
    ...
    def stage_result(self, result: WorkerResult, now: float) -> Task:
        """Persist worker output for Oracle review without completing the DAG task."""
        del now
        result_data = _normalise_result(result.task_id, result)
        with self._lock:
            task = self._require(result.task_id)
            if task.status is not TaskStatus.VERIFYING:
                raise InvalidTaskTransition(
                    f"task {result.task_id!r} must be VERIFYING before result staging"
                )
            _assert_result_claim(task, result_data)
            if task.result is None:
                task.result = result_data
            elif task.result != result_data:
                raise InvalidTaskTransition(
                    f"task {result.task_id!r} already has a different staged result"
                )
            return task.snapshot()
...
"""
    ...
    _STAGE_RESULT_SCRIPT = r"""
local raw = redis.call('GET', KEYS[1])
if not raw then return redis.error_reply('TASK_NOT_FOUND') end
local task = cjson.decode(raw)
if task.status ~= 'VERIFYING' then return redis.error_reply('INVALID_TRANSITION') end
local result = cjson.decode(ARGV[1])
if tonumber(result.attempt) ~= tonumber(task.attempt) then return redis.error_reply('STALE_ATTEMPT') end
if result.worker_id ~= task.worker_id then return redis.error_reply('FOREIGN_WORKER') end
if task.result and task.result ~= cjson.null then
  if cjson.encode(task.result) ~= cjson.encode(result) then return redis.error_reply('RESULT_CONFLICT') end
  return raw
end
task.result = result
local encoded = cjson.encode(task)
...
"""
    ...
    def stage_result(self, result: WorkerResult, now: float) -> Task:
        ...
        result_data = _normalise_result(result.task_id, result)
        response = self._eval(
            self._STAGE_RESULT_SCRIPT,
            [self._task_key(result.task_id)],
            [json.dumps(result_data, separators=(",", ":"))],
        )
        return self._task_required(response, result.task_id)
    ...
    def _eval(self, script: str, keys: list[str], arguments: list[Any]) -> Any:
        try:
            ...
            if any(code in message for code in ("INVALID_TRANSITION", "LEASE_EXPIRED", "STALE_ATTEMPT", "FOREIGN_WORKER", "RESULT_CONFLICT")):
